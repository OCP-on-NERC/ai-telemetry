...
import org.mghpcc.aitelemetry.model.hub.HubPage;
 ...
 * Generated: true
 **/
public class HubEnUSGenApiServiceImpl extends BaseApiServiceImpl implements HubEnUSGenApiService {

	protected static final Logger LOG = LoggerFactory.getLogger(HubEnUSGenApiServiceImpl.class);
		...
		user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
			...
			if("Inactive Token".equals(ex.getMessage()) || StringUtils.startsWith(ex.getMessage(), "invalid_grant:")) {
				try {
					...
					LOG.error(String.format("searchHub failed. ", ex2));
	...
	public Future<ServiceResponse> response200SearchHub(SearchList<Hub> listHub) {
		Promise<ServiceResponse> promise = Promise.promise();
		try {
			SiteRequest siteRequest = listHub.getSiteRequest_(SiteRequest.class);
			List<String> fls = listHub.getRequest().getFields();
			JsonObject json = new JsonObject();
			JsonArray l = new JsonArray();
			listHub.getList().stream().forEach(o -> {
				JsonObject json2 = JsonObject.mapFrom(o);
				if(fls.size() > 0) {
					Set<String> fieldNames = new HashSet<String>();
					for(String fieldName : json2.fieldNames()) {
						String v = Hub.varIndexedHub(fieldName);
						if(v != null)
							fieldNames.add(Hub.varIndexedHub(fieldName));
					}
	...
	public void responsePivotSearchHub(List<SolrResponse.Pivot> pivots, JsonArray pivotArray) {
		if(pivots != null) {
			for(SolrResponse.Pivot pivotField : pivots) {
				...
				if(pivotFields2 != null) {
					...
					responsePivotSearchHub(pivotFields2, pivotArray2);
				}
			}
		}
	}
		...
		user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
			...
			if("Inactive Token".equals(ex.getMessage()) || StringUtils.startsWith(ex.getMessage(), "invalid_grant:")) {
				try {
					...
					LOG.error(String.format("getHub failed. ", ex2));
					error(null, eventHandler, ex2);
				}
			} else if(StringUtils.startsWith(ex.getMessage(), "401 UNAUTHORIZED ")) {
				eventHandler.handle(Future.succeededFuture(
	...
	public Future<ServiceResponse> response200GETHub(SearchList<Hub> listHub) {
		Promise<ServiceResponse> promise = Promise.promise();
		try {
			SiteRequest siteRequest = listHub.getSiteRequest_(SiteRequest.class);
		...
		user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
			...
			if("Inactive Token".equals(ex.getMessage()) || StringUtils.startsWith(ex.getMessage(), "invalid_grant:")) {
				try {
					...
					LOG.error(String.format("patchHub failed. ", ex2));
					error(null, eventHandler, ex2);
				}
			} else if(StringUtils.startsWith(ex.getMessage(), "401 UNAUTHORIZED ")) {
				eventHandler.handle(Future.succeededFuture(
	...
	public Future<Void> listPATCHHub(ApiRequest apiRequest, SearchList<Hub> listHub) {
		Promise<Void> promise = Promise.promise();
		List<Future> futures = new ArrayList<>();
		SiteRequest siteRequest = listHub.getSiteRequest_(SiteRequest.class);
		listHub.getList().forEach(o -> {
			...
			futures.add(Future.future(promise1 -> {
				patchHubFuture(o2, false).onSuccess(a -> {
					promise1.complete();
				}).onFailure(ex -> {
		...
		CompositeFuture.all(futures).onSuccess( a -> {
			listHub.next().onSuccess(next -> {
				if(next) {
					listPATCHHub(apiRequest, listHub).onSuccess(b -> {
						promise.complete();
					}).onFailure(ex -> {
						LOG.error(String.format("listPATCHHub failed. "), ex);
						promise.fail(ex);
	...
	public Future<Hub> patchHubFuture(Hub o, Boolean inheritPrimaryKey) {
		...
		try {
			...
			Promise<Hub> promise1 = Promise.promise();
			pgPool.withTransaction(sqlConnection -> {
				siteRequest.setSqlConnection(sqlConnection);
				varsHub(siteRequest).onSuccess(a -> {
					sqlPATCHHub(o, inheritPrimaryKey).onSuccess(hub -> {
						persistHub(hub, true).onSuccess(c -> {
	...
	public Future<Hub> sqlPATCHHub(Hub o, Boolean inheritPrimaryKey) {
		Promise<Hub> promise = Promise.promise();
		try {
			SiteRequest siteRequest = o.getSiteRequest_();
			ApiRequest apiRequest = siteRequest.getApiRequest_();
			...
			Integer num = 1;
			StringBuilder bSql = new StringBuilder("UPDATE Hub SET ");
			...
			for(String entityVar : methodNames) {
				switch(entityVar) {
					case "setHubName":
							o2.setHubName(jsonObject.getString(entityVar));
							if(bParams.size() > 0)
								bSql.append(", ");
							bSql.append(Hub.VAR_hubName + "=$" + num);
							num++;
							bParams.add(o2.sqlHubName());
						break;
					case "setHubId":
							o2.setHubId(jsonObject.getString(entityVar));
							if(bParams.size() > 0)
								bSql.append(", ");
							bSql.append(Hub.VAR_hubId + "=$" + num);
							num++;
					...
					case "setPageId":
							...
							if(bParams.size() > 0)
								bSql.append(", ");
							bSql.append(Hub.VAR_pageId + "=$" + num);
							num++;
					...
					case "setUserKey":
							...
							if(bParams.size() > 0)
								bSql.append(", ");
							bSql.append(Hub.VAR_userKey + "=$" + num);
							num++;
					...
					case "setObjectTitle":
							...
							if(bParams.size() > 0)
								bSql.append(", ");
							bSql.append(Hub.VAR_objectTitle + "=$" + num);
							num++;
			...
			if(bParams.size() > 0) {
				...
				futures2.add(0, Future.future(a -> {
					sqlConnection.preparedQuery(bSql.toString())
						...
						RuntimeException ex2 = new RuntimeException("value Hub failed", ex);
						LOG.error(String.format("relateHub failed. "), ex2);
	...
	public Future<ServiceResponse> response200PATCHHub(SiteRequest siteRequest) {
		Promise<ServiceResponse> promise = Promise.promise();
		try {
			JsonObject json = new JsonObject();
			if(json == null) {
		...
		user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
			...
			if("Inactive Token".equals(ex.getMessage()) || StringUtils.startsWith(ex.getMessage(), "invalid_grant:")) {
				try {
					...
					LOG.error(String.format("postHub failed. ", ex2));
					error(null, eventHandler, ex2);
				}
			} else if(StringUtils.startsWith(ex.getMessage(), "401 UNAUTHORIZED ")) {
				eventHandler.handle(Future.succeededFuture(
	...
	public void postHubFuture(JsonObject body, ServiceRequest serviceRequest, Handler<AsyncResult<ServiceResponse>> eventHandler) {
		...
		user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
			try {
				...
				postHubFuture(siteRequest, false).onSuccess(o -> {
	...
	public Future<Hub> sqlPOSTHub(Hub o, Boolean inheritPrimaryKey) {
		Promise<Hub> promise = Promise.promise();
		try {
			SiteRequest siteRequest = o.getSiteRequest_();
			ApiRequest apiRequest = siteRequest.getApiRequest_();
	...
	public Future<ServiceResponse> response200POSTHub(Hub o) {
		Promise<ServiceResponse> promise = Promise.promise();
		try {
			SiteRequest siteRequest = o.getSiteRequest_();
		...
		user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
			...
			if("Inactive Token".equals(ex.getMessage()) || StringUtils.startsWith(ex.getMessage(), "invalid_grant:")) {
				try {
					...
					LOG.error(String.format("deleteHub failed. ", ex2));
					error(null, eventHandler, ex2);
				}
			} else if(StringUtils.startsWith(ex.getMessage(), "401 UNAUTHORIZED ")) {
				eventHandler.handle(Future.succeededFuture(
	...
	public Future<Void> listDELETEHub(ApiRequest apiRequest, SearchList<Hub> listHub) {
		Promise<Void> promise = Promise.promise();
		List<Future> futures = new ArrayList<>();
		SiteRequest siteRequest = listHub.getSiteRequest_(SiteRequest.class);
		listHub.getList().forEach(o -> {
		...
		CompositeFuture.all(futures).onSuccess( a -> {
			listHub.next().onSuccess(next -> {
				if(next) {
					listDELETEHub(apiRequest, listHub).onSuccess(b -> {
						promise.complete();
					}).onFailure(ex -> {
						LOG.error(String.format("listDELETEHub failed. "), ex);
						promise.fail(ex);
	...
	public Future<Hub> deleteHubFuture(Hub o) {
		SiteRequest siteRequest = o.getSiteRequest_();
		Promise<Hub> promise = Promise.promise();
		...
		try {
			...
			pgPool.withTransaction(sqlConnection -> {
				siteRequest.setSqlConnection(sqlConnection);
				varsHub(siteRequest).onSuccess(a -> {
					sqlDELETEHub(o).onSuccess(hub -> {
						relateHub(o).onSuccess(d -> {
							unindexHub(o).onSuccess(o2 -> {
								if(apiRequest != null) {
									apiRequest.setNumPATCH(apiRequest.getNumPATCH() + 1);
									if(apiRequest.getNumFound() == 1L && Optional.ofNullable(siteRequest.getJsonObject()).map(json -> json.size() > 0).orElse(false)) {
										o2.apiRequestHub();
	...
	public Future<Void> sqlDELETEHub(Hub o) {
		Promise<Void> promise = Promise.promise();
		try {
			SiteRequest siteRequest = o.getSiteRequest_();
			ApiRequest apiRequest = siteRequest.getApiRequest_();
			...
			SqlConnection sqlConnection = siteRequest.getSqlConnection();
			Integer num = 1;
			StringBuilder bSql = new StringBuilder("DELETE FROM Hub ");
			List<Object> bParams = new ArrayList<Object>();
			Long pk = o.getPk();
			JsonObject jsonObject = siteRequest.getJsonObject();
			Hub o2 = new Hub();
			...
			futures2.add(0, Future.future(a -> {
				sqlConnection.preparedQuery(bSql.toString())
					...
					RuntimeException ex2 = new RuntimeException("value Hub failed", ex);
					LOG.error(String.format("unrelateHub failed. "), ex2);
					a.handle(Future.failedFuture(ex2));
				});
			}));
			CompositeFuture.all(futures1).onSuccess(a -> {
	...
	public Future<ServiceResponse> response200DELETEHub(SiteRequest siteRequest) {
		Promise<ServiceResponse> promise = Promise.promise();
		try {
			JsonObject json = new JsonObject();
			if(json == null) {
		...
		user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
			...
			if("Inactive Token".equals(ex.getMessage()) || StringUtils.startsWith(ex.getMessage(), "invalid_grant:")) {
				try {
					...
					LOG.error(String.format("putimportHub failed. ", ex2));
					error(null, eventHandler, ex2);
				}
			} else if(StringUtils.startsWith(ex.getMessage(), "401 UNAUTHORIZED ")) {
				eventHandler.handle(Future.succeededFuture(
	...
	public Future<Void> listPUTImportHub(ApiRequest apiRequest, SiteRequest siteRequest) {
		Promise<Void> promise = Promise.promise();
		List<Future> futures = new ArrayList<>();
		JsonArray jsonArray = Optional.ofNullable(siteRequest.getJsonObject()).map(o -> o.getJsonArray("list")).orElse(new JsonArray());
		try {
			jsonArray.forEach(obj -> {
				futures.add(Future.future(promise1 -> {
					...
					JsonObject context = new JsonObject().put("params", params).put("user", siteRequest.getUserPrincipal());
					JsonObject json = new JsonObject().put("context", context);
					eventBus.request(Hub.getClassApiAddress(), json, new DeliveryOptions().addHeader("action", "putimportHubFuture")).onSuccess(a -> {
						promise1.complete();
					}).onFailure(ex -> {
									...
									for(Row definition : result.value()) {
										for(Integer i = 0; i < definition.size(); i++) {
											try {
												...
												LOG.error(String.format("persistHub failed. "), e);
											}
										}
									}
									Hub o2 = new Hub();
	...
	public Future<ServiceResponse> response200PUTImportHub(SiteRequest siteRequest) {
		Promise<ServiceResponse> promise = Promise.promise();
		try {
			JsonObject json = new JsonObject();
			if(json == null) {
		...
		user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
			...
			if("Inactive Token".equals(ex.getMessage()) || StringUtils.startsWith(ex.getMessage(), "invalid_grant:")) {
				try {
					...
					LOG.error(String.format("searchpageHub failed. ", ex2));
					error(null, eventHandler, ex2);
				}
			} else if(StringUtils.startsWith(ex.getMessage(), "401 UNAUTHORIZED ")) {
				eventHandler.handle(Future.succeededFuture(
	...
	public void responsePivotSearchPageHub(List<SolrResponse.Pivot> pivots, JsonArray pivotArray) {
		if(pivots != null) {
			for(SolrResponse.Pivot pivotField : pivots) {
				String entityIndexed = pivotField.getField();
				String entityVar = StringUtils.substringBefore(entityIndexed, "_docvalues_");
				...
				if(pivotFields2 != null) {
					JsonArray pivotArray2 = new JsonArray();
					pivotJson.put("pivot", pivotArray2);
					responsePivotSearchPageHub(pivotFields2, pivotArray2);
				}
			}
		}
	}
		...
		user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
			...
			if("Inactive Token".equals(ex.getMessage()) || StringUtils.startsWith(ex.getMessage(), "invalid_grant:")) {
				try {
					...
					LOG.error(String.format("editpageHub failed. ", ex2));
					error(null, eventHandler, ex2);
				}
			} else if(StringUtils.startsWith(ex.getMessage(), "401 UNAUTHORIZED ")) {
				eventHandler.handle(Future.succeededFuture(
	...
	public void responsePivotEditPageHub(List<SolrResponse.Pivot> pivots, JsonArray pivotArray) {
		if(pivots != null) {
			for(SolrResponse.Pivot pivotField : pivots) {
				String entityIndexed = pivotField.getField();
				String entityVar = StringUtils.substringBefore(entityIndexed, "_docvalues_");
				...
				if(pivotFields2 != null) {
					JsonArray pivotArray2 = new JsonArray();
					pivotJson.put("pivot", pivotArray2);
					responsePivotEditPageHub(pivotFields2, pivotArray2);
				}
			}
		}
	}
		...
		user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
			...
			if("Inactive Token".equals(ex.getMessage()) || StringUtils.startsWith(ex.getMessage(), "invalid_grant:")) {
				try {
					...
					LOG.error(String.format("deletefilterHub failed. ", ex2));
					error(null, eventHandler, ex2);
				}
			} else if(StringUtils.startsWith(ex.getMessage(), "401 UNAUTHORIZED ")) {
				eventHandler.handle(Future.succeededFuture(
	...
	public Future<Void> listDELETEFilterHub(ApiRequest apiRequest, SearchList<Hub> listHub) {
		Promise<Void> promise = Promise.promise();
		List<Future> futures = new ArrayList<>();
		SiteRequest siteRequest = listHub.getSiteRequest_(SiteRequest.class);
		listHub.getList().forEach(o -> {
			...
			o2.setSiteRequest_(siteRequest2);
			futures.add(Future.future(promise1 -> {
				deletefilterHubFuture(o).onSuccess(a -> {
					promise1.complete();
				}).onFailure(ex -> {
		...
		CompositeFuture.all(futures).onSuccess( a -> {
			listHub.next().onSuccess(next -> {
				if(next) {
					listDELETEFilterHub(apiRequest, listHub).onSuccess(b -> {
						promise.complete();
					}).onFailure(ex -> {
						LOG.error(String.format("listDELETEFilterHub failed. "), ex);
						promise.fail(ex);
	...
	public Future<Hub> deletefilterHubFuture(Hub o) {
		SiteRequest siteRequest = o.getSiteRequest_();
		Promise<Hub> promise = Promise.promise();
		...
		try {
			...
			pgPool.withTransaction(sqlConnection -> {
				siteRequest.setSqlConnection(sqlConnection);
				varsHub(siteRequest).onSuccess(a -> {
					sqlDELETEFilterHub(o).onSuccess(hub -> {
						relateHub(o).onSuccess(d -> {
							unindexHub(o).onSuccess(o2 -> {
								if(apiRequest != null) {
									apiRequest.setNumPATCH(apiRequest.getNumPATCH() + 1);
	...
	public Future<Void> sqlDELETEFilterHub(Hub o) {
		Promise<Void> promise = Promise.promise();
		try {
			SiteRequest siteRequest = o.getSiteRequest_();
			ApiRequest apiRequest = siteRequest.getApiRequest_();
	...
	public Future<ServiceResponse> response200DELETEFilterHub(SiteRequest siteRequest) {
		Promise<ServiceResponse> promise = Promise.promise();
		try {
			JsonObject json = new JsonObject();
			if(json == null) {
	...
	public Future<Hub> createHub(SiteRequest siteRequest) {
		Promise<Hub> promise = Promise.promise();
		try {
			SqlConnection sqlConnection = siteRequest.getSqlConnection();
			...
			sqlConnection.preparedQuery("INSERT INTO Hub(created, userKey) VALUES($1, $2) RETURNING pk")
	...
	public void searchHubQ(SearchList<Hub> searchList, String entityVar, String valueIndexed, String varIndexed) {
		searchList.q(varIndexed + ":" + ("*".equals(valueIndexed) ? valueIndexed : SearchTool.escapeQueryChars(valueIndexed)));
		if(!"*".equals(entityVar)) {
		}
	}

	public String searchHubFq(SearchList<Hub> searchList, String entityVar, String valueIndexed, String varIndexed) {
		if(varIndexed == null)
		...
		if(StringUtils.startsWith(valueIndexed, "[")) {
			...
			String fq1 = fqs[0].equals("*") ? fqs[0] : Hub.staticSearchFqForClass(entityVar, searchList.getSiteRequest_(SiteRequest.class), fqs[0]);
			String fq2 = fqs[1].equals("*") ? fqs[1] : Hub.staticSearchFqForClass(entityVar, searchList.getSiteRequest_(SiteRequest.class), fqs[1]);
			 return varIndexed + ":[" + fq1 + " TO " + fq2 + "]";
		} else {
			return varIndexed + ":" + SearchTool.escapeQueryChars(Hub.staticSearchFqForClass(entityVar, searchList.getSiteRequest_(SiteRequest.class), valueIndexed)).replace("\\", "\\\\");
		}
	}

	public void searchHubSort(SearchList<Hub> searchList, String entityVar, String valueIndexed, String varIndexed) {
		if(varIndexed == null)
			throw new RuntimeException(String.format("\"%s\" is not an indexed entity. ", entityVar));
		searchList.sort(varIndexed, valueIndexed);
	}

	public void searchHubRows(SearchList<Hub> searchList, Long valueRows) {
			searchList.rows(valueRows != null ? valueRows : 10L);
	}

	public void searchHubStart(SearchList<Hub> searchList, Long valueStart) {
		searchList.start(valueStart);
	}

	public void searchHubVar(SearchList<Hub> searchList, String var, String value) {
		searchList.getSiteRequest_(SiteRequest.class).getRequestVars().put(var, value);
	}

	public void searchHubUri(SearchList<Hub> searchList) {
	}

	public Future<ServiceResponse> varsHub(SiteRequest siteRequest) {
		Promise<ServiceResponse> promise = Promise.promise();
		try {
	...
	public Future<SearchList<Hub>> searchHubList(SiteRequest siteRequest, Boolean populate, Boolean store, Boolean modify) {
		Promise<SearchList<Hub>> promise = Promise.promise();
		try {
			ServiceRequest serviceRequest = siteRequest.getServiceRequest();
			...
			if(entityList != null) {
				for(String v : entityList) {
					searchList.fl(Hub.varIndexedHub(v));
				}
			}
					...
					if(paramValuesObject != null && "facet.pivot".equals(paramName)) {
						...
						if(mFacetPivot.find()) {
							...
							for(Integer i = 0; i < entityVars.length; i++) {
								...
								varsIndexed[i] = Hub.varIndexedHub(entityVar);
							}
						...
						for(Object paramObject : paramObjects) {
							if(paramName.equals("q")) {
								...
								while(mQ.find()) {
									...
									varIndexed = Hub.varIndexedHub(entityVar);
									String entityQ = searchHubFq(searchList, entityVar, valueIndexed, varIndexed);
								...
								while(mFq.find()) {
									...
									varIndexed = Hub.varIndexedHub(entityVar);
									String entityFq = searchHubFq(searchList, entityVar, valueIndexed, varIndexed);
								...
								varIndexed = Hub.varIndexedHub(entityVar);
								searchHubSort(searchList, entityVar, valueIndexed, varIndexed);
							} else if(paramName.equals("start")) {
								valueStart = paramObject instanceof Long ? (Long)paramObject : Long.parseLong(paramObject.toString());
								searchHubStart(searchList, valueStart);
							} else if(paramName.equals("rows")) {
								valueRows = paramObject instanceof Long ? (Long)paramObject : Long.parseLong(paramObject.toString());
								searchHubRows(searchList, valueRows);
								...
								entityVar = StringUtils.trim(StringUtils.substringBefore((String)paramObject, ":"));
								valueIndexed = URLDecoder.decode(StringUtils.trim(StringUtils.substringAfter((String)paramObject, ":")), "UTF-8");
								searchHubVar(searchList, entityVar, valueIndexed);
							} else if(paramName.equals("cursorMark")) {
								valueCursorMark = (String)paramObject;
								searchList.cursorMark((String)paramObject);
							}
						}
						searchHubUri(searchList);
					}
				} catch(Exception e) {
			...
			searchHub2(siteRequest, populate, store, modify, searchList);
	...
	public void searchHub2(SiteRequest siteRequest, Boolean populate, Boolean store, Boolean modify, SearchList<Hub> searchList) {
	}

	public Future<Void> persistHub(Hub o, Boolean patch) {
		Promise<Void> promise = Promise.promise();
		try {
			SiteRequest siteRequest = o.getSiteRequest_();
			SqlConnection sqlConnection = siteRequest.getSqlConnection();
	...
	public Future<Void> relateHub(Hub o) {
		Promise<Void> promise = Promise.promise();
		promise.complete();
		return promise.future();
	}
	...
	public String searchVar(String varIndexed) {
		return Hub.searchVarHub(varIndexed);
	}

	@Override
	public String getClassApiAddress() {
		return Hub.CLASS_API_ADDRESS_Hub;
	}

	public Future<Hub> indexHub(Hub o) {
		Promise<Hub> promise = Promise.promise();
		try {
			SiteRequest siteRequest = o.getSiteRequest_();
			ApiRequest apiRequest = siteRequest.getApiRequest_();
	...
	public Future<Hub> unindexHub(Hub o) {
		Promise<Hub> promise = Promise.promise();
		try {
			SiteRequest siteRequest = o.getSiteRequest_();
			ApiRequest apiRequest = siteRequest.getApiRequest_();
			o.promiseDeepForClass(siteRequest).onSuccess(a -> {
				...
				String query = String.format("filter(%s:%s)", Hub.VAR_solrId, o.obtainForClass(Hub.VAR_solrId));
	...
	public Future<Void> refreshHub(Hub o) {
		Promise<Void> promise = Promise.promise();
		SiteRequest siteRequest = o.getSiteRequest_();
		try {
			ApiRequest apiRequest = siteRequest.getApiRequest_();
			...
			if(refresh && !Optional.ofNullable(siteRequest.getJsonObject()).map(JsonObject::isEmpty).orElse(true)) {
				...
				CompositeFuture.all(futures).onSuccess(b -> {
					...
					JsonObject context = new JsonObject().put("params", params).put("user", siteRequest.getUserPrincipal());
					JsonObject json = new JsonObject().put("context", context);
					eventBus.request(Hub.getClassApiAddress(), json, new DeliveryOptions().addHeader("action", "patchHubFuture")).onSuccess(c -> {
