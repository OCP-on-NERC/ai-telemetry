 ...
 * Generated: true
 **/
public class ClusterOrderGenPage extends ClusterOrderGenPageGen<PageLayout> {

  /**
   * {@inheritDoc}
   * Ignore: true
   **/
  protected void _searchListClusterOrder_(Wrap<SearchList<ClusterOrder>> w) {
  }
  ...
  protected void _varsQ(JsonObject vars) {
    ClusterOrder.varsQForClass().forEach(var -> {
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(ClusterOrder.displayNameClusterOrder(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(ClusterOrder.classSimpleNameClusterOrder(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", Optional.ofNullable(searchListClusterOrder_.getRequest().getQuery()).filter(fq -> fq.startsWith(ClusterOrder.varIndexedClusterOrder(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    varsFqCount = 0;
    for(String var : ClusterOrder.varsFqForClass()) {
      String varIndexed = ClusterOrder.varIndexedClusterOrder(var);
      String varStored = ClusterOrder.varStoredClusterOrder(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      ...
      json.put("displayName", Optional.ofNullable(ClusterOrder.displayNameClusterOrder(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(ClusterOrder.classSimpleNameClusterOrder(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      Object v = searchListClusterOrder_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(ClusterOrder.varIndexedClusterOrder(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null);
      if(v != null) {
        json.put("val", v);
        varsFqCount++;
      }
  ...
  protected void _varsRange(JsonObject vars) {
    ClusterOrder.varsRangeForClass().forEach(var -> {
      String varIndexed = ClusterOrder.varIndexedClusterOrder(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(ClusterOrder.displayNameClusterOrder(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(ClusterOrder.classSimpleNameClusterOrder(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", searchListClusterOrder_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(ClusterOrder.varIndexedClusterOrder(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _query(JsonObject query) {
    ...
    Long rows1 = Optional.ofNullable(searchListClusterOrder_).map(l -> l.getRows()).orElse(10L);
    Long start1 = Optional.ofNullable(searchListClusterOrder_).map(l -> l.getStart()).orElse(1L);
    ...
    for(String fq : Optional.ofNullable(searchListClusterOrder_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
      if(!StringUtils.contains(fq, "(")) {
        String fq1 = ClusterOrder.searchVarClusterOrder(StringUtils.substringBefore(fq, ":"));
        String fq2 = StringUtils.substringAfter(fq, ":");
        if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "sessionId", "userKeys"))
          fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", ClusterOrder.displayNameForClass(fq1)));
        }
      }
    query.put("fq", fqs);

    JsonArray sorts = new JsonArray();
    for(String sort : Optional.ofNullable(searchListClusterOrder_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
      String sort1 = ClusterOrder.searchVarClusterOrder(StringUtils.substringBefore(sort, " "));
      sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", ClusterOrder.displayNameForClass(sort1)));
    }
  ...
  protected void _rangeGap(Wrap<String> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
      w.o(Optional.ofNullable(searchListClusterOrder_.getFacetRangeGap()).orElse(null));
  }
  ...
  protected void _rangeEnd(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
      w.o(Optional.ofNullable(searchListClusterOrder_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _rangeStart(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
      w.o(Optional.ofNullable(searchListClusterOrder_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _defaultRangeVar(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListClusterOrder_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return ClusterOrder.searchVarClusterOrder(v); }).orElse("created"));
  }

  @Override
  protected void _defaultFacetSort(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListClusterOrder_.getFacetSort()).orElse("index"));
  }

  @Override
  protected void _defaultFacetLimit(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListClusterOrder_.getFacetLimit()).orElse(1));
  }

  @Override
  protected void _defaultFacetMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListClusterOrder_.getFacetMinCount()).orElse(1));
  }

  @Override
  protected void _defaultPivotMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListClusterOrder_.getFacetPivotMinCount()).orElse(0));
  }
  ...
  protected void _defaultSortVars(List<String> l) {
    if(!searchListClusterOrder_.getDefaultSort()) {
      Optional.ofNullable(searchListClusterOrder_.getSorts()).orElse(Arrays.asList()).forEach(varSortStr -> {
        String varSortParts[] = varSortStr.split(" ");
        String varSort = ClusterOrder.searchVarClusterOrder(varSortParts[0]);
  ...
  protected void _defaultFieldListVars(List<String> l) {
    Optional.ofNullable(searchListClusterOrder_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
      ...
      for(String part : parts) {
        if(StringUtils.isNotBlank(part)) {
          String var = ClusterOrder.searchVarClusterOrder(part);
  ...
  protected void _defaultStatsVars(List<String> l) {
    Optional.ofNullable(searchListClusterOrder_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
  ...
  protected void _defaultPivotVars(List<String> l) {
    Optional.ofNullable(searchListClusterOrder_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
   ...
   * {@inheritDoc}
   **/
  protected void _listClusterOrder(JsonArray l) {
    Optional.ofNullable(searchListClusterOrder_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
  }

  protected void _resultCount(Wrap<Integer> w) {
    w.o(searchListClusterOrder_ == null ? 0 : searchListClusterOrder_.size());
  }

  /**
   * Initialized: false
  **/
  protected void _result(Wrap<ClusterOrder> w) {
    if(resultCount >= 1 && Optional.ofNullable(siteRequest_.getServiceRequest().getParams().getJsonObject("path")).map(o -> o.getString("id")).orElse(null) != null)
      w.o(searchListClusterOrder_.get(0));
  }
  ...
  protected void _pageUri(Wrap<String> c) {
    c.o("/en-us/search/cluster-order");
  }

  @Override
  protected void _apiUri(Wrap<String> c) {
    c.o("/en-us/api/cluster-order");
  }
  ...
  protected void _pageImageUri(Wrap<String> c) {
      c.o("/png/en-us/search/cluster-order-999.png");
  }
  ...
  protected void _pageUriClusterOrder(Wrap<String> c) {
      c.o("/en-us/search/cluster-order");
  }
}
