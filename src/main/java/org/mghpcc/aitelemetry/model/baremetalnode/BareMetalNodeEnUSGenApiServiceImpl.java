...
public class BareMetalNodeEnUSGenApiServiceImpl extends BaseApiServiceImpl implements BareMetalNodeEnUSGenApiService {
				...
				searchList.promiseDeepForClass(siteRequest).onSuccess(a -> {
					try {
						if(searchList.size() >= 1) {
							BareMetalNode o = searchList.getList().stream().findFirst().orElse(null);
							BareMetalNode o2 = new BareMetalNode();
							o2.setSiteRequest_(siteRequest);
	...
	public Future<Void> persistBareMetalNode(BareMetalNode o, Boolean patch) {
		...
		try {
			...
			SqlConnection sqlConnection = siteRequest.getSqlConnection();
			Long pk = o.getPk();
			sqlConnection.preparedQuery("SELECT * FROM BareMetalNode WHERE pk=$1")
					.collecting(Collectors.toList())
					.execute(Tuple.of(pk)
					).onSuccess(result -> {
				try {
	...
	public Future<JsonObject> generatePageBody(ComputateSiteRequest siteRequest, Map<String, Object> ctx, String templatePath, String classSimpleName) {
		...
		try {
			...
			page.persistForClass(BareMetalNode.VAR_created, BareMetalNode.staticSetCreated(siteRequest2, (String)result.get(BareMetalNode.VAR_created)));
