 ...
 * Generated: true
 **/
public class BareMetalOrderGenPage extends BareMetalOrderGenPageGen<PageLayout> {

  /**
   * {@inheritDoc}
   * Ignore: true
   **/
  protected void _searchListBareMetalOrder_(Wrap<SearchList<BareMetalOrder>> w) {
  }
  ...
  protected void _varsQ(JsonObject vars) {
    BareMetalOrder.varsQForClass().forEach(var -> {
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(BareMetalOrder.displayNameBareMetalOrder(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(BareMetalOrder.classSimpleNameBareMetalOrder(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", Optional.ofNullable(searchListBareMetalOrder_.getRequest().getQuery()).filter(fq -> fq.startsWith(BareMetalOrder.varIndexedBareMetalOrder(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    varsFqCount = 0;
    for(String var : BareMetalOrder.varsFqForClass()) {
      String varIndexed = BareMetalOrder.varIndexedBareMetalOrder(var);
      String varStored = BareMetalOrder.varStoredBareMetalOrder(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      ...
      json.put("displayName", Optional.ofNullable(BareMetalOrder.displayNameBareMetalOrder(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(BareMetalOrder.classSimpleNameBareMetalOrder(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      Object v = searchListBareMetalOrder_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(BareMetalOrder.varIndexedBareMetalOrder(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null);
      if(v != null) {
        json.put("val", v);
        varsFqCount++;
      }
  ...
  protected void _varsRange(JsonObject vars) {
    BareMetalOrder.varsRangeForClass().forEach(var -> {
      String varIndexed = BareMetalOrder.varIndexedBareMetalOrder(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(BareMetalOrder.displayNameBareMetalOrder(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(BareMetalOrder.classSimpleNameBareMetalOrder(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", searchListBareMetalOrder_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(BareMetalOrder.varIndexedBareMetalOrder(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _query(JsonObject query) {
    ...
    Long rows1 = Optional.ofNullable(searchListBareMetalOrder_).map(l -> l.getRows()).orElse(10L);
    Long start1 = Optional.ofNullable(searchListBareMetalOrder_).map(l -> l.getStart()).orElse(1L);
    ...
    for(String fq : Optional.ofNullable(searchListBareMetalOrder_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
      if(!StringUtils.contains(fq, "(")) {
        String fq1 = BareMetalOrder.searchVarBareMetalOrder(StringUtils.substringBefore(fq, ":"));
        String fq2 = StringUtils.substringAfter(fq, ":");
        if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "sessionId", "userKeys"))
          fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", BareMetalOrder.displayNameForClass(fq1)));
        }
      }
    query.put("fq", fqs);

    JsonArray sorts = new JsonArray();
    for(String sort : Optional.ofNullable(searchListBareMetalOrder_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
      String sort1 = BareMetalOrder.searchVarBareMetalOrder(StringUtils.substringBefore(sort, " "));
      sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", BareMetalOrder.displayNameForClass(sort1)));
    }
  ...
  protected void _rangeGap(Wrap<String> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
      w.o(Optional.ofNullable(searchListBareMetalOrder_.getFacetRangeGap()).orElse(null));
  }
  ...
  protected void _rangeEnd(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
      w.o(Optional.ofNullable(searchListBareMetalOrder_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _rangeStart(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
      w.o(Optional.ofNullable(searchListBareMetalOrder_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _defaultRangeVar(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListBareMetalOrder_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return BareMetalOrder.searchVarBareMetalOrder(v); }).orElse("created"));
  }

  @Override
  protected void _defaultFacetSort(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListBareMetalOrder_.getFacetSort()).orElse("index"));
  }

  @Override
  protected void _defaultFacetLimit(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListBareMetalOrder_.getFacetLimit()).orElse(1));
  }

  @Override
  protected void _defaultFacetMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListBareMetalOrder_.getFacetMinCount()).orElse(1));
  }

  @Override
  protected void _defaultPivotMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListBareMetalOrder_.getFacetPivotMinCount()).orElse(0));
  }
  ...
  protected void _defaultSortVars(List<String> l) {
    if(!searchListBareMetalOrder_.getDefaultSort()) {
      Optional.ofNullable(searchListBareMetalOrder_.getSorts()).orElse(Arrays.asList()).forEach(varSortStr -> {
        String varSortParts[] = varSortStr.split(" ");
        String varSort = BareMetalOrder.searchVarBareMetalOrder(varSortParts[0]);
  ...
  protected void _defaultFieldListVars(List<String> l) {
    Optional.ofNullable(searchListBareMetalOrder_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
      ...
      for(String part : parts) {
        if(StringUtils.isNotBlank(part)) {
          String var = BareMetalOrder.searchVarBareMetalOrder(part);
  ...
  protected void _defaultStatsVars(List<String> l) {
    Optional.ofNullable(searchListBareMetalOrder_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
  ...
  protected void _defaultPivotVars(List<String> l) {
    Optional.ofNullable(searchListBareMetalOrder_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
   ...
   * {@inheritDoc}
   **/
  protected void _listBareMetalOrder(JsonArray l) {
    Optional.ofNullable(searchListBareMetalOrder_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
  }

  protected void _resultCount(Wrap<Integer> w) {
    w.o(searchListBareMetalOrder_ == null ? 0 : searchListBareMetalOrder_.size());
  }

  /**
   * Initialized: false
  **/
  protected void _result(Wrap<BareMetalOrder> w) {
    if(resultCount >= 1 && Optional.ofNullable(siteRequest_.getServiceRequest().getParams().getJsonObject("path")).map(o -> o.getString("pk")).orElse(null) != null)
      w.o(searchListBareMetalOrder_.get(0));
  }
  ...
  protected void _pageTitle(Wrap<String> c) {
    if(result != null && result.getObjectTitle() != null)
      ...
      c.o("bare metal orders");
    else if(searchListBareMetalOrder_ == null || resultCount == 0)
      c.o("no bare metal order found");
    else
      c.o("bare metal orders");
  }
  ...
  protected void _pageDescription(Wrap<String> c) {
      c.o("A bare metal order");
  }

  @Override
  protected void _pageImageUri(Wrap<String> c) {
      c.o("/png/en-us/search/bare-metal-order-999.png");
  }
