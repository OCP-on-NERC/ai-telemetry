 ...
 * Generated: true
 **/
public class BareMetalResourceClassGenPage extends BareMetalResourceClassGenPageGen<PageLayout> {

  /**
   * {@inheritDoc}
   * Ignore: true
   **/
  protected void _searchListBareMetalResourceClass_(Wrap<SearchList<BareMetalResourceClass>> w) {
  }
  ...
  protected void _varsQ(JsonObject vars) {
    BareMetalResourceClass.varsQForClass().forEach(var -> {
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(BareMetalResourceClass.displayNameBareMetalResourceClass(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(BareMetalResourceClass.classSimpleNameBareMetalResourceClass(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", Optional.ofNullable(searchListBareMetalResourceClass_.getRequest().getQuery()).filter(fq -> fq.startsWith(BareMetalResourceClass.varIndexedBareMetalResourceClass(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    varsFqCount = 0;
    for(String var : BareMetalResourceClass.varsFqForClass()) {
      String varIndexed = BareMetalResourceClass.varIndexedBareMetalResourceClass(var);
      String varStored = BareMetalResourceClass.varStoredBareMetalResourceClass(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      ...
      json.put("displayName", Optional.ofNullable(BareMetalResourceClass.displayNameBareMetalResourceClass(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(BareMetalResourceClass.classSimpleNameBareMetalResourceClass(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      Object v = searchListBareMetalResourceClass_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(BareMetalResourceClass.varIndexedBareMetalResourceClass(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null);
      if(v != null) {
        json.put("val", v);
        varsFqCount++;
      }
  ...
  protected void _varsRange(JsonObject vars) {
    BareMetalResourceClass.varsRangeForClass().forEach(var -> {
      String varIndexed = BareMetalResourceClass.varIndexedBareMetalResourceClass(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(BareMetalResourceClass.displayNameBareMetalResourceClass(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(BareMetalResourceClass.classSimpleNameBareMetalResourceClass(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", searchListBareMetalResourceClass_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(BareMetalResourceClass.varIndexedBareMetalResourceClass(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _query(JsonObject query) {
    ...
    Long rows1 = Optional.ofNullable(searchListBareMetalResourceClass_).map(l -> l.getRows()).orElse(10L);
    Long start1 = Optional.ofNullable(searchListBareMetalResourceClass_).map(l -> l.getStart()).orElse(1L);
    ...
    for(String fq : Optional.ofNullable(searchListBareMetalResourceClass_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
      if(!StringUtils.contains(fq, "(")) {
        String fq1 = BareMetalResourceClass.searchVarBareMetalResourceClass(StringUtils.substringBefore(fq, ":"));
        String fq2 = StringUtils.substringAfter(fq, ":");
        if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "sessionId", "userKeys"))
          fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", BareMetalResourceClass.displayNameForClass(fq1)));
        }
      }
    query.put("fq", fqs);

    JsonArray sorts = new JsonArray();
    for(String sort : Optional.ofNullable(searchListBareMetalResourceClass_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
      String sort1 = BareMetalResourceClass.searchVarBareMetalResourceClass(StringUtils.substringBefore(sort, " "));
      sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", BareMetalResourceClass.displayNameForClass(sort1)));
    }
  ...
  protected void _rangeGap(Wrap<String> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
      w.o(Optional.ofNullable(searchListBareMetalResourceClass_.getFacetRangeGap()).orElse(null));
  }
  ...
  protected void _rangeEnd(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
      w.o(Optional.ofNullable(searchListBareMetalResourceClass_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _rangeStart(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
      w.o(Optional.ofNullable(searchListBareMetalResourceClass_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _defaultRangeVar(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListBareMetalResourceClass_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return BareMetalResourceClass.searchVarBareMetalResourceClass(v); }).orElse("created"));
  }

  @Override
  protected void _defaultFacetSort(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListBareMetalResourceClass_.getFacetSort()).orElse("index"));
  }

  @Override
  protected void _defaultFacetLimit(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListBareMetalResourceClass_.getFacetLimit()).orElse(1));
  }

  @Override
  protected void _defaultFacetMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListBareMetalResourceClass_.getFacetMinCount()).orElse(1));
  }

  @Override
  protected void _defaultPivotMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListBareMetalResourceClass_.getFacetPivotMinCount()).orElse(0));
  }
  ...
  protected void _defaultSortVars(List<String> l) {
    if(!searchListBareMetalResourceClass_.getDefaultSort()) {
      Optional.ofNullable(searchListBareMetalResourceClass_.getSorts()).orElse(Arrays.asList()).forEach(varSortStr -> {
        String varSortParts[] = varSortStr.split(" ");
        String varSort = BareMetalResourceClass.searchVarBareMetalResourceClass(varSortParts[0]);
  ...
  protected void _defaultFieldListVars(List<String> l) {
    Optional.ofNullable(searchListBareMetalResourceClass_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
      ...
      for(String part : parts) {
        if(StringUtils.isNotBlank(part)) {
          String var = BareMetalResourceClass.searchVarBareMetalResourceClass(part);
  ...
  protected void _defaultStatsVars(List<String> l) {
    Optional.ofNullable(searchListBareMetalResourceClass_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
  ...
  protected void _defaultPivotVars(List<String> l) {
    Optional.ofNullable(searchListBareMetalResourceClass_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
   ...
   * {@inheritDoc}
   **/
  protected void _listBareMetalResourceClass(JsonArray l) {
    Optional.ofNullable(searchListBareMetalResourceClass_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
  }

  protected void _resultCount(Wrap<Integer> w) {
    w.o(searchListBareMetalResourceClass_ == null ? 0 : searchListBareMetalResourceClass_.size());
  }

  /**
   * Initialized: false
  **/
  protected void _result(Wrap<BareMetalResourceClass> w) {
    if(resultCount >= 1 && Optional.ofNullable(siteRequest_.getServiceRequest().getParams().getJsonObject("path")).map(o -> o.getString("name")).orElse(null) != null)
      w.o(searchListBareMetalResourceClass_.get(0));
  }
  ...
  protected void _pageUri(Wrap<String> c) {
    c.o("/en-us/search/bare-metal-resource-class");
  }

  @Override
  protected void _apiUri(Wrap<String> c) {
    c.o("/en-us/api/bare-metal-resource-class");
  }
  ...
  protected void _pageDescription(Wrap<String> c) {
      c.o("A bare metal resource class");
  }

  @Override
  protected void _pageImageUri(Wrap<String> c) {
      c.o("/png/en-us/search/bare-metal-resource-class-999.png");
  }
  ...
  protected void _pageUriBareMetalResourceClass(Wrap<String> c) {
      c.o("/en-us/search/bare-metal-resource-class");
  }
}
