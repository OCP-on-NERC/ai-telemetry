...
public class ClusterTemplateEnUSGenApiServiceImpl extends BaseApiServiceImpl implements ClusterTemplateEnUSGenApiService {
					...
					if(authorizationDecisionResponse.failed() && !scopes.contains("PATCH")) {
						...
						searchClusterTemplateList(siteRequest, false, true, true).onSuccess(listClusterTemplate -> {
							try {
								...
								apiRequest.setSolrId(Optional.ofNullable(listClusterTemplate.first()).map(o2 -> o2.getSolrId()).orElse(null));
				...
				try {
					...
					if(authorizationDecisionResponse.failed() && !scopes.contains("POST")) {
						...
						eventBus.request(ClusterTemplate.getClassApiAddress(), json, new DeliveryOptions().addHeader("action", "postClusterTemplateFuture")).onSuccess(a -> {
							...
							apiRequest.setSolrId(responseBody.getString(ClusterTemplate.VAR_solrId));
	...
	public Future<Void> persistClusterTemplate(ClusterTemplate o, Boolean patch) {
		...
		try {
			...
			SqlConnection sqlConnection = siteRequest.getSqlConnection();
			Long pk = o.getPk();
			sqlConnection.preparedQuery("SELECT id, title, created, description, parameters, archived, sessionId, userKey, objectTitle, displayPage FROM ClusterTemplate WHERE pk=$1")
					.collecting(Collectors.toList())
					.execute(Tuple.of(pk)
					).onSuccess(result -> {
				try {
	...
	public Future<JsonObject> generatePageBody(ComputateSiteRequest siteRequest, Map<String, Object> ctx, String templatePath, String classSimpleName) {
		...
		try {
			...
			page.persistForClass(ClusterTemplate.VAR_created, ClusterTemplate.staticSetCreated(siteRequest2, (String)result.get(ClusterTemplate.VAR_created), Optional.ofNullable(siteRequest).map(r -> r.getConfig()).map(config -> config.getString(ConfigKeys.SITE_ZONE)).map(z -> ZoneId.of(z)).orElse(ZoneId.of("UTC"))));
