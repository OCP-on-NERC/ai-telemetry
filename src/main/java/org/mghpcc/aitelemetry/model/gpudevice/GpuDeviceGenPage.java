 ...
 * Generated: true
 **/
public class GpuDeviceGenPage extends GpuDeviceGenPageGen<PageLayout> {

  /**
   * {@inheritDoc}
   * Ignore: true
   **/
  protected void _searchListGpuDevice_(Wrap<SearchList<GpuDevice>> w) {
  }

  @Override
  protected void _pageResponse(Wrap<String> w) {
    if(searchListGpuDevice_ != null)
      w.o(Optional.ofNullable(searchListGpuDevice_.getResponse()).map(response -> JsonObject.mapFrom(response).toString()).orElse(null));
  }

  @Override
  protected void _stats(Wrap<SolrResponse.Stats> w) {
    w.o(Optional.ofNullable(searchListGpuDevice_.getResponse()).map(response -> response.getStats()).orElse(null));
  }

  @Override
  protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
    w.o(Optional.ofNullable(searchListGpuDevice_.getResponse()).map(response -> response.getFacetCounts()).orElse(null));
  }

  @Override
  protected void _pagination(JsonObject pagination) {
    JsonArray pages = new JsonArray();
    Long start = searchListGpuDevice_.getStart().longValue();
    Long rows = searchListGpuDevice_.getRows().longValue();
    Long foundNum = Optional.ofNullable(searchListGpuDevice_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListGpuDevice_.getList().size()));
  ...
  protected void _varsQ(JsonObject vars) {
    GpuDevice.varsQForClass().forEach(var -> {
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(GpuDevice.displayNameGpuDevice(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(GpuDevice.classSimpleNameGpuDevice(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", Optional.ofNullable(searchListGpuDevice_.getRequest().getQuery()).filter(fq -> fq.startsWith(GpuDevice.varIndexedGpuDevice(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    for(String var : GpuDevice.varsFqForClass()) {
      ...
      json.put("displayName", Optional.ofNullable(GpuDevice.displayNameGpuDevice(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(GpuDevice.classSimpleNameGpuDevice(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      Object v = searchListGpuDevice_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(GpuDevice.varIndexedGpuDevice(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null);
      if(v != null) {
        json.put("val", v);
        varsFqCount++;
      }
  ...
  protected void _varsRange(JsonObject vars) {
    GpuDevice.varsRangeForClass().forEach(var -> {
      String varIndexed = GpuDevice.varIndexedGpuDevice(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(GpuDevice.displayNameGpuDevice(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(GpuDevice.classSimpleNameGpuDevice(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", searchListGpuDevice_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(GpuDevice.varIndexedGpuDevice(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _query(JsonObject query) {
    ...
    Long num = Optional.ofNullable(searchListGpuDevice_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListGpuDevice_.getList().size()));
    String q = "*:*";
    ...
    Long rows1 = Optional.ofNullable(searchListGpuDevice_).map(l -> l.getRows()).orElse(10L);
    Long start1 = Optional.ofNullable(searchListGpuDevice_).map(l -> l.getStart()).orElse(1L);
    ...
    for(String fq : Optional.ofNullable(searchListGpuDevice_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
      if(!StringUtils.contains(fq, "(")) {
        String fq1 = GpuDevice.searchVarGpuDevice(StringUtils.substringBefore(fq, ":"));
        String fq2 = StringUtils.substringAfter(fq, ":");
        if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "sessionId", "userKeys"))
          fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", GpuDevice.displayNameForClass(fq1)));
        }
      }
    query.put("fq", fqs);

    JsonArray sorts = new JsonArray();
    for(String sort : Optional.ofNullable(searchListGpuDevice_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
      String sort1 = GpuDevice.searchVarGpuDevice(StringUtils.substringBefore(sort, " "));
      sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", GpuDevice.displayNameForClass(sort1)));
    }
  ...
  protected void _rangeGap(Wrap<String> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
      w.o(Optional.ofNullable(searchListGpuDevice_.getFacetRangeGap()).orElse(null));
  }
  ...
  protected void _rangeEnd(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
      w.o(Optional.ofNullable(searchListGpuDevice_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _rangeStart(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
      w.o(Optional.ofNullable(searchListGpuDevice_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _defaultRangeVar(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListGpuDevice_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return GpuDevice.searchVarGpuDevice(v); }).orElse("created"));
  }

  @Override
  protected void _defaultFacetSort(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListGpuDevice_.getFacetSort()).orElse("index"));
  }

  @Override
  protected void _defaultFacetLimit(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListGpuDevice_.getFacetLimit()).orElse(1));
  }

  @Override
  protected void _defaultFacetMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListGpuDevice_.getFacetMinCount()).orElse(1));
  }

  @Override
  protected void _defaultPivotMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListGpuDevice_.getFacetPivotMinCount()).orElse(0));
  }

  @Override
  protected void _DEFAULT_MAP_LOCATION(Wrap<JsonObject> w) {
    Point point = GpuDevice.staticSetLocation(siteRequest_, Optional.ofNullable(siteRequest_.getRequestVars().get(VAR_DEFAULT_MAP_LOCATION)).orElse(siteRequest_.getConfig().getString(ConfigKeys.DEFAULT_MAP_LOCATION)));
  ...
  protected void _defaultSortVars(List<String> l) {
    if(!searchListGpuDevice_.getDefaultSort()) {
      Optional.ofNullable(searchListGpuDevice_.getSorts()).orElse(Arrays.asList()).forEach(varSortStr -> {
        String varSortParts[] = varSortStr.split(" ");
        String varSort = GpuDevice.searchVarGpuDevice(varSortParts[0]);
  ...
  protected void _defaultFieldListVars(List<String> l) {
    Optional.ofNullable(searchListGpuDevice_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
      ...
      for(String part : parts) {
        if(StringUtils.isNotBlank(part)) {
          String var = GpuDevice.searchVarGpuDevice(part);
  ...
  protected void _defaultStatsVars(List<String> l) {
    Optional.ofNullable(searchListGpuDevice_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
  ...
  protected void _defaultPivotVars(List<String> l) {
    Optional.ofNullable(searchListGpuDevice_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
   ...
   * {@inheritDoc}
   **/
  protected void _listGpuDevice(JsonArray l) {
    Optional.ofNullable(searchListGpuDevice_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
  }

  protected void _resultCount(Wrap<Integer> w) {
    w.o(searchListGpuDevice_ == null ? 0 : searchListGpuDevice_.size());
  }
  ...
  **/
  ...
  protected void _pageTitle(Wrap<String> c) {
    if(result != null && result.getObjectTitle() != null)
      ...
      c.o("GPU devices");
    else if(searchListGpuDevice_ == null || resultCount == 0)
      c.o("no GPU device found");
    else
      c.o("GPU devices");
  }
  ...
  protected void _pageDescription(Wrap<String> c) {
      c.o("A Red Hat OpenShift GPU device containing GPUs");
  }

  @Override
  protected void _pageImageUri(Wrap<String> c) {
      c.o("/png/en-us/search/gpu-device-999.png");
  }

  @Override
  protected void _classIcon(Wrap<String> c) {
      c.o("<i class=\"fa-regular fa-memory\"></i>");
  }

  protected void _pageUriGpuDevice(Wrap<String> c) {
      c.o("/en-us/search/gpu-device");
  }
}
