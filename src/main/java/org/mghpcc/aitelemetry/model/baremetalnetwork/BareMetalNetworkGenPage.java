 ...
 * Generated: true
 **/
public class BareMetalNetworkGenPage extends BareMetalNetworkGenPageGen<PageLayout> {

  /**
   * {@inheritDoc}
   * Ignore: true
   **/
  protected void _searchListBareMetalNetwork_(Wrap<SearchList<BareMetalNetwork>> w) {
  }

  @Override
  protected void _pageResponse(Wrap<String> w) {
    if(searchListBareMetalNetwork_ != null)
      w.o(Optional.ofNullable(searchListBareMetalNetwork_.getResponse()).map(response -> JsonObject.mapFrom(response).toString()).orElse(null));
  }

  @Override
  protected void _stats(Wrap<SolrResponse.Stats> w) {
    w.o(Optional.ofNullable(searchListBareMetalNetwork_.getResponse()).map(response -> response.getStats()).orElse(null));
  }

  @Override
  protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
    w.o(Optional.ofNullable(searchListBareMetalNetwork_.getResponse()).map(response -> response.getFacetCounts()).orElse(null));
  }

  @Override
  protected void _pagination(JsonObject pagination) {
    JsonArray pages = new JsonArray();
    Long start = searchListBareMetalNetwork_.getStart().longValue();
    Long rows = searchListBareMetalNetwork_.getRows().longValue();
    Long foundNum = Optional.ofNullable(searchListBareMetalNetwork_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListBareMetalNetwork_.getList().size()));
  ...
  protected void _varsQ(JsonObject vars) {
    BareMetalNetwork.varsQForClass().forEach(var -> {
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(BareMetalNetwork.displayNameBareMetalNetwork(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(BareMetalNetwork.classSimpleNameBareMetalNetwork(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", Optional.ofNullable(searchListBareMetalNetwork_.getRequest().getQuery()).filter(fq -> fq.startsWith(BareMetalNetwork.varIndexedBareMetalNetwork(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    varsFqCount = 0;
    for(String var : BareMetalNetwork.varsFqForClass()) {
      String varIndexed = BareMetalNetwork.varIndexedBareMetalNetwork(var);
      String varStored = BareMetalNetwork.varStoredBareMetalNetwork(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      ...
      json.put("displayName", Optional.ofNullable(BareMetalNetwork.displayNameBareMetalNetwork(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(BareMetalNetwork.classSimpleNameBareMetalNetwork(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      Object v = searchListBareMetalNetwork_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(BareMetalNetwork.varIndexedBareMetalNetwork(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null);
      if(v != null) {
        json.put("val", v);
        varsFqCount++;
      }
  ...
  protected void _varsRange(JsonObject vars) {
    BareMetalNetwork.varsRangeForClass().forEach(var -> {
      String varIndexed = BareMetalNetwork.varIndexedBareMetalNetwork(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(BareMetalNetwork.displayNameBareMetalNetwork(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(BareMetalNetwork.classSimpleNameBareMetalNetwork(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", searchListBareMetalNetwork_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(BareMetalNetwork.varIndexedBareMetalNetwork(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _query(JsonObject query) {
    ...
    Long num = Optional.ofNullable(searchListBareMetalNetwork_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListBareMetalNetwork_.getList().size()));
    String q = "*:*";
    ...
    Long rows1 = Optional.ofNullable(searchListBareMetalNetwork_).map(l -> l.getRows()).orElse(10L);
    Long start1 = Optional.ofNullable(searchListBareMetalNetwork_).map(l -> l.getStart()).orElse(1L);
    ...
    for(String fq : Optional.ofNullable(searchListBareMetalNetwork_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
      if(!StringUtils.contains(fq, "(")) {
        String fq1 = BareMetalNetwork.searchVarBareMetalNetwork(StringUtils.substringBefore(fq, ":"));
        String fq2 = StringUtils.substringAfter(fq, ":");
        if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "sessionId", "userKeys"))
          fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", BareMetalNetwork.displayNameForClass(fq1)));
        }
      }
    query.put("fq", fqs);

    JsonArray sorts = new JsonArray();
    for(String sort : Optional.ofNullable(searchListBareMetalNetwork_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
      String sort1 = BareMetalNetwork.searchVarBareMetalNetwork(StringUtils.substringBefore(sort, " "));
      sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", BareMetalNetwork.displayNameForClass(sort1)));
    }
  ...
  protected void _rangeGap(Wrap<String> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
      w.o(Optional.ofNullable(searchListBareMetalNetwork_.getFacetRangeGap()).orElse(null));
  }
  ...
  protected void _rangeEnd(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
      w.o(Optional.ofNullable(searchListBareMetalNetwork_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _rangeStart(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
      w.o(Optional.ofNullable(searchListBareMetalNetwork_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _defaultRangeVar(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListBareMetalNetwork_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return BareMetalNetwork.searchVarBareMetalNetwork(v); }).orElse("created"));
  }

  @Override
  protected void _defaultFacetSort(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListBareMetalNetwork_.getFacetSort()).orElse("index"));
  }

  @Override
  protected void _defaultFacetLimit(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListBareMetalNetwork_.getFacetLimit()).orElse(1));
  }

  @Override
  protected void _defaultFacetMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListBareMetalNetwork_.getFacetMinCount()).orElse(1));
  }

  @Override
  protected void _defaultPivotMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListBareMetalNetwork_.getFacetPivotMinCount()).orElse(0));
  }
  ...
  protected void _defaultSortVars(List<String> l) {
    if(!searchListBareMetalNetwork_.getDefaultSort()) {
      Optional.ofNullable(searchListBareMetalNetwork_.getSorts()).orElse(Arrays.asList()).forEach(varSortStr -> {
        String varSortParts[] = varSortStr.split(" ");
        String varSort = BareMetalNetwork.searchVarBareMetalNetwork(varSortParts[0]);
  ...
  protected void _defaultFieldListVars(List<String> l) {
    Optional.ofNullable(searchListBareMetalNetwork_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
      ...
      for(String part : parts) {
        if(StringUtils.isNotBlank(part)) {
          String var = BareMetalNetwork.searchVarBareMetalNetwork(part);
  ...
  protected void _defaultStatsVars(List<String> l) {
    Optional.ofNullable(searchListBareMetalNetwork_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
  ...
  protected void _defaultPivotVars(List<String> l) {
    Optional.ofNullable(searchListBareMetalNetwork_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
   ...
   * {@inheritDoc}
   **/
  protected void _listBareMetalNetwork(JsonArray l) {
    Optional.ofNullable(searchListBareMetalNetwork_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
  }

  protected void _resultCount(Wrap<Integer> w) {
    w.o(searchListBareMetalNetwork_ == null ? 0 : searchListBareMetalNetwork_.size());
  }

  /**
   * Initialized: false
  **/
  protected void _result(Wrap<BareMetalNetwork> w) {
    if(resultCount >= 1 && Optional.ofNullable(siteRequest_.getServiceRequest().getParams().getJsonObject("path")).map(o -> o.getString("id")).orElse(null) != null)
      w.o(searchListBareMetalNetwork_.get(0));
  }
  ...
  protected void _pageTitle(Wrap<String> c) {
    if(result != null && result.getObjectTitle() != null)
      ...
      c.o("bare metal networks");
    else if(searchListBareMetalNetwork_ == null || resultCount == 0)
      c.o("no bare metal network found");
    else
      c.o("bare metal networks");
  }
  ...
  protected void _pageDescription(Wrap<String> c) {
      c.o("An OpenStack bare metal network");
  }

  @Override
  protected void _pageImageUri(Wrap<String> c) {
      c.o("/png/en-us/search/bare-metal-network-999.png");
  }
  ...
  protected void _pageUriBareMetalNetwork(Wrap<String> c) {
      c.o("/en-us/search/bare-metal-network");
  }
}
