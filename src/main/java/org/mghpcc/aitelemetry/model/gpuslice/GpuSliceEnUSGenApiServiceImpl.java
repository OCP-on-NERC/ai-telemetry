		...
		user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
			try {
				...
				pgPool.getConnection().onSuccess(sqlConnection -> {
					String sqlQuery = String.format("select * from %s WHERE sliceName=$1", GpuSlice.CLASS_SIMPLE_NAME);
	...
	public Future<GpuSlice> unindexGpuSlice(GpuSlice o) {
		...
		try {
			...
			o.promiseDeepForClass(siteRequest).onSuccess(a -> {
				...
				String query = String.format("filter(%s:%s)", GpuSlice.VAR_solrId, o.obtainForClass(GpuSlice.VAR_solrId));
