...
public class AiClusterEnUSGenApiServiceImpl extends BaseApiServiceImpl implements AiClusterEnUSGenApiService {
	...
	public Future<Void> persistAiCluster(AiCluster o, Boolean patch) {
		...
		try {
			...
			SqlConnection sqlConnection = siteRequest.getSqlConnection();
			Long pk = o.getPk();
			sqlConnection.preparedQuery("SELECT clusterName, description, created, archived, location, id, sessionId, ngsildTenant, userKey, ngsildPath, ngsildContext, ngsildData, objectTitle, aiNodesTotal, displayPage, gpuDevicesTotal FROM AiCluster WHERE pk=$1")
					.collecting(Collectors.toList())
					.execute(Tuple.of(pk)
					).onSuccess(result -> {
				try {
