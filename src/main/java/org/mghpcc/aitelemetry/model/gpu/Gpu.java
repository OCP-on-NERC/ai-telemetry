 ...
 * Indexed: true
 * Description: A GPU inside a GPU device
 * 
 * ApiTag: GPU
 * ApiUri: /api/gpu
 * 
 ...
 * AName: a GPU
 * PluralName: GPUs
 * Icon: <i class="fa-regular fa-microchip"></i>
 */
public class Gpu extends GpuGen<BaseModel> {
   ...
   * Persist: true
   * DisplayName: name
   * Description: The name of this GPU
   * HtmRow: 3
   * HtmCell: 1
   * HtmColumn: 1
  ...
  protected void _objectTitle(Wrap<String> w) {
    ...
    b.append(Optional.ofNullable(entityShortId).map(s -> String.format("%s — %s", Gpu_NameAdjectiveSingular_enUS, s)).orElse(pk.toString()));
    w.o(b.toString().trim());
  }
